...
多平台（飞书 & Telegram & 企业微信）机器人 ↔ Claude Code CLI 桥接服务，支持 MCP 协议。

用户在飞书、Telegram 或企业微信中发消息，服务器接收后调用 Claude Code 执行，并将输出实时流式推送回聊天窗口。

同时提供 MCP Server 模式，可通过 stdio 与 Claude Code 等 MCP 客户端集成。
...
cc-im 提供 MCP (Model Context Protocol) Server，可通过 stdio 与 Claude Code 等 MCP 客户端集成。MCP 模式支持飞书、Telegram 和企业微信三个平台。

```bash
# 启动 MCP Server（通过环境变量或配置文件）
export WECOM_BOT_ID=your_bot_id
export WECOM_BOT_SECRET=your_bot_secret
cc-im mcp
```
...
| 工具 | 说明 |
|------|------|
| `send_message` | 向指定平台（飞书/Telegram/企业微信）的群或个人发送消息 |
| `get_active_chats` | 获取最近活跃的聊天列表（可按平台筛选） |
| `get_chat_info` | 获取指定聊天的详细信息 |
| `get_incoming_messages` | 轮询获取收到的消息（可按平台筛选） |

**工具参数说明：**

- `send_message` 需要指定 `platform` 参数（`wecom` / `feishu` / `telegram`）、`chatId` 和 `content`
- `get_active_chats` 和 `get_incoming_messages` 可选指定 `platform` 筛选
- `get_chat_info` 需要指定 `platform` 和 `chatId`
...
├── mcp/
│   ├── index.ts             # MCP 模块导出
│   ├── cli.ts               # MCP 独立模式入口（cc-im mcp）
│   ├── server.ts            # MCP Bridge Server（多平台工具 + 消息处理器注册）
│   ├── router.ts            # 跨平台消息路由器（ChatInfo/Message 接口 + MessageRouter）
│   └── tools.ts             # MCP 工具定义（send_message/get_active_chats 等）
├── shared/
...
本项目诞生于 [congqiu/cc-im](https://github.com/congqiu/cc-im) 的坚实基础之上——原项目构建了飞书、Telegram 与企业微信三大平台与 Claude Code 之间的消息桥梁，让 AI 助手得以融入日常沟通场景。

在此基础上，我们实现了 **MCP (Model Context Protocol)** 协议扩展，赋予 Claude Code 主动向办公软件推送消息的能力，使 AI 与团队协作工具之间的交互真正成为双向闭环。
